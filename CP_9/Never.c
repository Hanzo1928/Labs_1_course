...
int main(void)
{
    ...
    Key key;
    char filename_val[40];                                          // Данные считываемого файла
    char filename_key[40];                                          // Ключи считываемого файла
    printf("Enter the name of the file with keys:\n");
    scanf("%s", filename_key);                                          // Считываем название
    FILE *file_key = fopen(filename_key, "r");                   // Открываем файл на чтение

    if (file_key == NULL)                                               // Проверка файла на пустоту
    {
    ...
    printf("Enter the name of the file with values:\n");
    scanf("%s", filename_val);                                          // Считываем название
    FILE *file_val = fopen(filename_val, "r");                   // Открываем файл на чтение

    if (file_val == NULL)                                               // Проверка файла на пустоту
    {
        printf("Ошибка при открытии файла\n");
    ...
    while (i < N && fscanf(file_key, "%d", &keys[i].key) == 1)          // Пока номер строки меньше максимального (N) и fscanf считала значение (вернула 1)
    {
        fscanf(file_val, "%c", &ch);                                    // Считываем символ из файла
        getRow(file_val, values[i].val, sizeof(values[i].val));     // Запись считанных символов строки из потока в строку Val

        i++;                                                        // Увеличиваем счетчик считанных строк
    }

    fclose(file_key);                                                   // Закрываем файл
    fclose(file_val);                                                   // Закрываем файл
